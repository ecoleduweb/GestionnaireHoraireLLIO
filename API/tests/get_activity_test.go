...
func TestUsersActivities(t *testing.T) {
	w := sendRequest(router, "GET", "/activities/me", nil, doNotDeleteUser.Id)
	assertResponse(t, w, http.StatusOK, nil)
	assert.NotNil(t, w.Body)
}

func TestGetActivity(t *testing.T) {
	w := sendRequest(router, "GET", fmt.Sprintf("/activity/%d", doNotDeleteActivity.Id), nil, doNotDeleteUser.Id)
	assertResponse(t, w, http.StatusOK, nil)
	assert.NotNil(t, w.Body)
}

func TestGetNotFoundActivity(t *testing.T) {
	w := sendRequest(router, "GET", "/activity/0", nil, doNotDeleteUser.Id)
